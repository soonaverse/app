...
@UntilDestroy()
@Component({
  selector: 'wen-locked-token-claim',
  templateUrl: './locked-token-claim.component.html',
  styleUrls: ['./locked-token-claim.component.less'],
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class LockedTokenClaimComponent implements OnInit, OnDestroy {
  ...
  public ngOnInit(): void {
    ...
    this.transaction$.pipe(untilDestroyed(this)).subscribe((val) => {
      if (val && val.type === TransactionType.ORDER) {
        ...
        if (expiresOn.isBefore(dayjs()) || val.payload?.void || val.payload?.reconciled) {
          this.transaction?.uid && removeLockedTokenClaimItem(this.transaction.uid);
        }
      ...
      if (
        ...
        setTimeout(() => {
          ...
          this.purchasedAmount = Number(val.payload.amount);
    ...
    if (this.transaction?.uid && getLockedTokenClaimItem(this.transaction.uid)) {
      this.transSubscription = this.orderApi
        .listen(<string>getLockedTokenClaimItem(this.transaction.uid))
        .subscribe(<any>this.transaction$);
    }
  ...
  public async claimToken(): Promise<void> {
    ...
    await this.auth.sign(params, (sc, finish) => {
      this.notification
        .processRequest(
          this.transactionApi.creditUnrefundable(sc),
          $localize`Credit refund submitted.`,
          finish,
        )
        .subscribe((val: any) => {
          this.transSubscription?.unsubscribe();
          this.transaction?.uid && setLockedTokenClaimItem(this.transaction.uid, val.uid);
