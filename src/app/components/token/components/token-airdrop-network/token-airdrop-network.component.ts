...
export class TokenAirdropNetworkComponent implements OnInit {
  ...
  public close(): void {
    ...
    this.router.navigate(['/', ROUTER_UTILS.config.token.root, this.token?.uid]);
  }
  ...
  public async proceedWithAirdrop(): Promise<void> {
    if (!this.token || !this.token?.mintingData) {
      return;
    }
    ...
    await this.auth.sign(params, (sc, finish) => {
      this.notification
        .processRequest(this.tokenApi.airdropMintedToken(sc), 'Order created.', finish)
        .subscribe((val: any) => {
          this.transSubscription?.unsubscribe();
          setItem(StorageItem.TokenAirdropTransaction, val.uid);
